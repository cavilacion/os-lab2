...
int isProcessStillAlive(int pid){
...
void sigChldHandler(int sig, siginfo_t *si, void *unused) {
    ...
    if (isProcessStillAlive(si->si_pid)){ // If the process is no longer alive its page was already retrieved
        printf("Retrieving from child\n");
        retrieveUpToDateListFromChild(si->si_pid);
    } else {
        printf("Retrieving from child not needed\n");
    }
}
